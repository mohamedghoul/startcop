...
class QCBKnowledgeBaseScraper:
    ...
    def _scrape_category(self, url: str, category: str) -> List[Dict[str, Any]]:
        ...
        try:
            resp = requests.get(
                url, timeout=30, headers={"User-Agent": "RegulatoryNavigator/1.0"}
            )
    ...
    def _parse_fintech_regulations(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        regulations = []
        containers = soup.find_all("div", class_=lambda x: x and "content" in x) or [
            soup
        ]
    ...
    def _parse_legislation(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for section in soup.find_all(
            lambda t: t.name == "div" and "legislation" in (t.get("class", []) or [])
        ):
            for item in section.find_all(
                ["div", "li", "p"], class_=lambda x: x and "item" in x
            ):
                    ...
                    regulations.append(
                        {
                            ...
                            "extracted_requirements": self._extract_structured_requirements(
                                content
                            ),
                        }
                    )
    ...
    def _parse_security_requirements(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for section in soup.find_all(["div", "section"]):
            if not any(
                term in section.get_text("", "").lower()
                for term in ["security", "cyber", "information", "data protection"]
            ):
                continue
    ...
    def _parse_aml_requirements(self, soup: BeautifulSoup) -> List[Dict[str, Any]]:
        ...
        for section in soup.find_all(["div", "section"]):
            if not any(
                term in section.get_text("", "").lower()
                for term in ["aml", "cft", "anti-money", "laundering"]
            ):
                continue
    ...
    def _parse_licensing_requirements(
                    ...
                    regulations.append(
                        {
                            ...
                            "summary": self._generate_summary(txt),
                            "extracted_requirements": {
                                "capital_requirement": self._extract_capital_requirement(
                                    txt
                                ),
                                "license_fees": self._extract_license_fees(txt),
                                "compliance_requirements": self._extract_structured_requirements(
                                    txt
                                ).get(
    ...
    def _parse_pdf_regulation(self, url: str, category: str) -> List[Dict[str, Any]]:
        try:
            resp = requests.get(
                url, timeout=60, headers={"User-Agent": "RegulatoryNavigator/1.0"}
            )
            resp.raise_for_status()
        except Exception as e:
    ...
    def _extract_security_title(self, text: str) -> str:
        match = re.search(
            r"(information security|cyber security|data protection)[\s:–-]*([A-Z].{0,80})",
            text,
            re.I,
        )
    ...
    def _extract_aml_title(self, text: str) -> str:
        match = re.search(
            r"(AML|anti-money laundering|CFT)[\s:–-]*([A-Z].{0,80})", text, re.I
        )
    ...
    def _extract_structured_requirements(self, text: str) -> Dict[str, Any]:
        ...
        capital_match = re.search(
            r"(?:minimum\s+)?(?:paid-up|share|authorized)\s+capital\s+(?:of\s+)?(?:QAR|\$)?\s*([\d,]+(?:\.\d{2})?)",
            text,
            re.I,
        )
        if capital_match:
        ...
        limit_match = re.search(
            r"(?:maximum|up to|limit of)\s+(?:QAR|\$)?\s*([\d,]+(?:\.\d{2})?)",
            text,
            re.I,
        )
        if limit_match:
...
if __name__ == "__main__":
    ...
    print(
        f"Total regulations scraped: {knowledge_base['metadata']['total_regulations']}"
    )
